import { Icon, Box, Image, Skeleton } from '@chakra-ui/react';
...
const LogoFallback = ({ isCollapsed, isSmall }: { isCollapsed?: boolean; isSmall?: boolean }) => {
  ...
  const logoColor = useExtendedColorMode('blue.600', 'white');
...
const NetworkLogo = ({ isCollapsed, onClick }: Props) => {
  ...
  const logoSrc = useExtendedColorMode(config.UI.sidebar.logo.default, config.UI.sidebar.logo.dark || config.UI.sidebar.logo.default);
  const iconSrc = useExtendedColorMode(config.UI.sidebar.icon.default, config.UI.sidebar.icon.dark || config.UI.sidebar.icon.default);
  const darkModeFilter = { filter: 'brightness(0) invert(1)' };
  const logoStyle = useExtendedColorMode({}, !config.UI.sidebar.logo.dark ? darkModeFilter : {});
  const iconStyle = useExtendedColorMode({}, !config.UI.sidebar.icon.dark ? darkModeFilter : {});
