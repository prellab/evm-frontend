import { InputGroup, Input, InputLeftElement, Icon, chakra, forwardRef, InputRightElement } from '@chakra-ui/react';
...
const SearchBarInput = ({ onChange, onSubmit, isHomepage, onFocus, onBlur, onHide, onClear, value }: Props, ref: React.ForwardedRef<HTMLFormElement>) => {
  ...
  const bgColor = useExtendedColorMode('white', 'black');
    ...
    >
      <InputGroup size={{ base: isHomepage ? 'md' : 'sm', lg: 'md' }}>
        <InputLeftElement w={{ base: isHomepage ? 6 : 4, lg: 6 }} ml={{ base: isHomepage ? 4 : 3, lg: 4 }} h="100%">
          <Icon as={ searchIcon } boxSize={{ base: isHomepage ? 6 : 4, lg: 6 }} color={ useExtendedColorMode('blackAlpha.600', 'whiteAlpha.600') }/>
        </InputLeftElement>
        <Input
          ...
          borderColor={ useExtendedColorMode('blackAlpha.100', 'whiteAlpha.200') }
          _focusWithin={{ _placeholder: { color: 'gray.300' } }}
          color={ useExtendedColorMode('black', 'white') }
          value={ value }
        />
        { value && (
