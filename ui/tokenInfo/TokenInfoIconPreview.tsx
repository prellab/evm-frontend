...
const TokenInfoIconPreview = ({ url, onError, onLoad, isInvalid }: Props) => {
  const borderColor = useExtendedColorMode('gray.100', 'gray.700');
  const borderColorFilled = useExtendedColorMode('gray.300', 'gray.600');
  const borderColorError = useExtendedColorMode('red.400', 'red.300');
