...
import { Skeleton, Flex, chakra, LightMode } from '@chakra-ui/react';
...
const StatsItem = ({ icon, title, value, className, tooltipLabel, url, isLoading }: Props) => {
  ...
  const loadingBgColor = useExtendedColorMode('blackAlpha.50', 'whiteAlpha.50');
  const infoColor = useExtendedColorMode('gray.600', 'gray.400');
  ...
  return (
    ...
    >
      ...
      >
        ...
        <Skeleton isLoaded={ !isLoading } fontWeight={ 500 } fontSize="md" color={ useExtendedColorMode('black', 'white') } borderRadius="base">
          <span>{ value }</span>
        </Skeleton>
      </Flex>
