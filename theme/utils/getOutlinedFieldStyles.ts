...
export default function getOutlinedFieldStyles(props: StyleFunctionProps) {
  ...
  return {
    ...
    borderColor: useExtendedColorMode('gray.300', 'gray.600'),
    ...transitionProps,
    _hover: {
      borderColor: useExtendedColorMode('gray.200', 'gray.500'),
    },
    _readOnly: {
    ...
    _disabled: {
      opacity: 1,
      backgroundColor: useExtendedColorMode('blackAlpha.200', 'whiteAlpha.200'),
      ...
      ':-webkit-autofill': {
        ...
        '-webkit-box-shadow': `0 0 0px 1000px ${ useExtendedColorMode('rgba(16, 17, 18, 0.08)', 'rgba(255, 255, 255, 0.08)') } inset`,
      },
    },
    _invalid: {
    ...
    _placeholder: {
      color: useExtendedColorMode('blackAlpha.600', 'whiteAlpha.600'),
    },
    // not filled input
    ':placeholder-shown:not(:focus-visible):not(:hover):not([aria-invalid=true])': { borderColor: borderColor || useExtendedColorMode('gray.100', 'gray.700') },
